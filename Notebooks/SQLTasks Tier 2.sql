...
cursorObj= con.execute("SELECT f.name, sum(CASE WHEN b.memid = 0 THEN guestcost WHEN b.memid > 0 THEN membercost END) AS totalcost FROM `Facilities` f, `Bookings` b WHERE f.facid = b. facid GROUP BY f.name HAVING totalcost < 1000 ORDER BY totalcost")
